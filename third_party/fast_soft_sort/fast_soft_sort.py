...
def isotonic_l2(input_s, input_w=None):
  ...
  isoisotonic_l2(input_s - input_w, solution)
  return solution
...
def isotonic_kl(input_s, input_w=None):
  ...
  isoisotonic_kl(input_s, input_w.astype(input_s.dtype), solution)
  return solution
...
### added for boosting
def soft_sort_jac(values, direction="ASCENDING", regularization_strength=1.0,
              regularization="l2"):
    """Computes Jacobian."""
    identity = np.eye(values.size)
    obje=SoftSort(values, regularization_strength=regularization_strength,
                  direction=direction, regularization=regularization)
    comp=obje.compute()
    return np.array([obje.jvp(identity[i]) for i in range(len(identity))]).T

def soft_sort_vjp(values, vector, direction="ASCENDING", regularization_strength=1.0,
              regularization="l2"):
    """Computes Jacobian."""
    obje=SoftSort(values, regularization_strength=regularization_strength,
                  direction=direction, regularization=regularization)
    comp=obje.compute()
    return obje.vjp(vector)
